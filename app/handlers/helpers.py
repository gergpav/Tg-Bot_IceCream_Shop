...
def build_order_buttons(order_id: int, items: list[Product]) -> InlineKeyboardMarkup:
    ...
    for item in items:
        keyboard.append([InlineKeyboardButton(f"{item.name}",
                                              callback_data=f"add_item|{order_id}|{item.id}")])
    keyboard.append([InlineKeyboardButton("Завершить заказ", callback_data=f"finish_order|{order_id}")])
